...
describe('calculateElapsedPlayheadTime', () => {
  ...
  it('returns 12.012 for seg-7.m4s', () => {
    assert.equal(
      calculateElapsedPlayheadTime('seg-7.m4s'),
      SEG.mul(6).toNumber()
    );
  });
  ...
  it('returns 4.004 for seg-high-3.m4s', () => {
    assert.equal(
      calculateElapsedPlayheadTime('seg-high-3.m4s'),
      SEG.mul(2).toNumber()
    );
  });
...
describe('roundToSegmentSize', () => {
  ...
  it('3 → 4.004', () =>
    assert.equal(roundToSegmentSize(3), SEG.mul(2).toNumber()));
  it('0 → 0', () => assert.equal(roundToSegmentSize(0), 0));
  it('30.03 → 30.03', () =>
    assert.equal(roundToSegmentSize(30.03), SEG.mul(15).toNumber()));
});
