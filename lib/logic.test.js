...
describe('resolveRenditionErrors', () => {
  it('returns empty playlist and segment maps when no rendition errors', () => {
    const cues = [{ cue: 'startup', delay: 5 }, { cue: 'playback', time: 30 }];
    assert.deepEqual(resolveRenditionErrors(cues), { playlist: {}, segment: {} });
  });

  it('puts error with no "on" field and no prior playback into playlist', () => {
    const cues = [{ cue: 'error', code: 404, rendition: 'low' }];
    assert.deepEqual(resolveRenditionErrors(cues), { playlist: { low: 404 }, segment: {} });
  });
  ...
  it('converts error with no "on" field and prior playback to timed segment error', () => {
    ...
    assert.deepEqual(resolveRenditionErrors(cues), { playlist: {}, segment: { low: { code: 404, activateAtSegment: 6 } } });
  });
  ...
  it('puts error with on:segment into segment with activateAtSegment:1 when no prior playback', () => {
    ...
    assert.deepEqual(resolveRenditionErrors(cues), { playlist: {}, segment: { mid: { code: 503, activateAtSegment: 1 } } });
  });
  ...
  it('puts error with on:segment into segment with correct activateAtSegment after playback', () => {
    ...
    assert.deepEqual(resolveRenditionErrors(cues), { playlist: {}, segment: { low: { code: 404, activateAtSegment: 4 } } });
  });

  it('defaults to 500 when no code specified', () => {
    const cues = [{ cue: 'error', rendition: 'low' }];
    assert.deepEqual(resolveRenditionErrors(cues), { playlist: { low: 500 }, segment: {} });
  });
  ...
  it('handles multiple rendition errors across both targets', () => {
    ...
    assert.deepEqual(resolveRenditionErrors(cues), { playlist: { low: 404 }, segment: { mid: { code: 503, activateAtSegment: 2 } } });
  });
